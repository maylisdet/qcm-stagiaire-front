...
const QuizzContent = () => {
  ...
  const columns = [
    ...
    { field: 'edit', headerName: 'Edit', with: 600,  
      renderCell: () => 
        {
          return (
            <Button onClick= {({quizz_id= 2, question_id= 3}) => {history.push(`/admin/quizz/${quizz_id}/question/${question_id}/edit`)}}> Edit</Button>
          )
        }
    },
    ...
    return (
      <Container>
        <Stack spacing={6} mt={4}>
          <TextField id="standard-basic" label="Name" variant="standard" value={values.name} onChange={handleChange('name')}/>
